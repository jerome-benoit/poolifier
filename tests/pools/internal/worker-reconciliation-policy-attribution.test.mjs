...
it.each([
  ...
  ['detached', false, true],
])('scopes raw crash details for reserved %s tasks', async (
  previousState,
  retainsCause,
  recoverable
) => {
  ...
  if (retainsCause) {
    expect(rejected.message).toBe('Worker node crashed: reservation raw sentinel')
...
it.each([
...
])('attributes raw crash details to one active reservation: %j', async (
  previousStates,
  rawTaskId
) => {
  const taskIds = previousStates.map((_, index) =>
    `00000000-0000-0000-0000-000000000d0${index + 1}`
  )
  const lease = { generation: 6, id: 23 }
  const rawCause = new Error('multi-reservation raw sentinel')
  const hooks = createHooks()
  hooks.reserve.mockReturnValue(previousStates.map((previousState, index) => ({
    lease,
    previousState,
    taskId: taskIds[index],
  })))
  const policy = new WorkerReconciliationPolicy(hooks)
  const worker = { info: { dynamic: false, id: lease.id }, usage: { tasks: {} } }
  ...
  for (const taskId of taskIds) {
    ...
    expect(rejected.signal).toBe('SIGTERM')
    if (taskId === rawTaskId) {
      expect(rejected.message).toBe('Worker node crashed: multi-reservation raw sentinel')
      expect(rejected.cause).toBe(rawCause)
    } else {
      expect(rejected.message).toBe('Worker node crashed')
      expect(Object.hasOwn(rejected, 'cause')).toBe(false)
