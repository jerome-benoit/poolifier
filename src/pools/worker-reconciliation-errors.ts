...
export const buildWorkerCrashError = (
  ...
  taskId?: TaskUUID
): WorkerCrashError => cause instanceof WorkerCrashError
...
): WorkerCrashError => isCrashAttributed
...
export const buildWorkerReconciliationError = <Worker extends LifecycleWorker>(
  ...
  const cause = transition.cause instanceof WorkerCrashError
    ? transition.cause
    : transition.cause instanceof Error
      ? new WorkerCrashError(`Worker node crashed: ${transition.cause.message}`, {
